...
Проект содержит два самостоятельных блока решения:

- обработку XML-аннотаций в папке task1;
- работу с COCO-датасетом и его преобразование в YOLO в папке task2.

Решение сделал так, чтобы оно было понятно, воспроизводимо и легко запускалось на локальной машине.

Разработчик: Ордынцев Виктор Игоревич
Email: ordyntsev.vk@yandex.ru (предпочтительный способ связи)
Tel: +79375664396

## Что реализовано

- Анализ XML-файлов: статистика по изображениям, классам, фигурам и типам объектов.
- Модификация XML-аннотаций с сохранением новых копий.
- Реструктуризация COCO-датасета по классам.
- Валидация датасета и генерация отчёта.
...
Test_task_Data_Engineering/
    ...
    │   ├── input_data/               # входные XML-файлы
    │   ├── script1.py                # общая статистика по XML
    │   ├── script2.py                # статистика по классам
    │   ├── script3.py                # статистика по типам фигур
    │   └── script4.py                # модификация XML-файлов
    ├── task2/                        # обработка COCO-датасета
    │   ├── input_data/               # входные данные COCO
    │   ├── script1.py                # реструктуризация датасета
    │   ├── script2.py                # валидация и отчёт
    │   ├── script3.py                # преобразование в YOLO
    │   ├── updated_data/             # результаты после script1/script2
    │   └── yolo_dataset/             # результат преобразования в YOLO
